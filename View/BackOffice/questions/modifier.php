...
$pageTitle = "Modifier la Question - HearMe Admin";
$additionalCss = '<link rel="stylesheet" href="/hearme_user/View/BackOffice/assets/css/admin-questions.css">';
include __DIR__ . '/../layout/header.php';
?>

<div class="page-header">
    <a href="/hearme_user/Controller/QuestionController.php?action=liste&id_quiz=<?= $question['id_quiz'] ?>" class="btn-dark mb-3" style="display: inline-flex; align-items: center; gap: 8px;">
    ...
    <p>Modifier les informations de la question</p>
</div>

<div class="row justify-content-center">
    <div class="col-lg-10">
        <div class="form-dark full-width">
            <form action="/hearme_user/Controller/QuestionController.php?action=update&id=<?= $question['id_question'] ?>" method="POST">
                ...
                <div class="row mb-4">
                    <div class="col-md-9">
                        ...
                        <textarea name="texte_question" class="form-control" rows="2" required><?= e($question['texte_question']) ?></textarea>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Ordre</label>
                        <input type="number" name="ordre" class="form-control" value="<?= $question['ordre'] ?>" min="1">
                    </div>
                </div>
                ...
                <div class="option-card">
                    ...
                    <div class="row">
                        <div class="col-md-9">
                            <input type="text" name="option<?= $i ?>_texte" class="form-control" 
                                   value="<?= e($question["option{$i}_texte"] ?? '') ?>" <?= $i <= 3 ? 'required' : '' ?>>
                        </div>
                        <div class="col-md-3">
                            <input type="number" name="option<?= $i ?>_score" class="form-control" 
                                   value="<?= $question["option{$i}_score"] ?? $i ?>" min="1" max="10">
                        </div>
                    </div>
                </div>
                <?php endfor; ?>
                ...
                <div class="d-flex gap-3 mt-4">
                    ...
                    <a href="/hearme_user/Controller/QuestionController.php?action=liste&id_quiz=<?= $question['id_quiz'] ?>" class="btn-dark">Annuler</a>
                </div>
            </form>
        </div>
    </div>
